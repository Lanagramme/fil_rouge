...
const 
	...
	search   = (par) => { query = par; api.get(data4) },
...
data4.callback = (res) => {
	const response = Object.values(res.response)
	resultat = response.filter(x => x.nom.toLowerCase().indexOf(query) !== -1)
	cl(JSON.stringify(resultat))
}

data2.type = data3.type = data4.type = 'entreprise'
